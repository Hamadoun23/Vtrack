...
@section('title', 'Nouveau véhicule - Vtrack')
    ...
    <div class="col-md-10 offset-md-1">
        <div class="card">
            <div class="card-header">
                <i class="bi bi-car-front"></i> Nouveau véhicule
            </div>
            <div class="card-body">
                <form action="{{ route('vehicules.store') }}" method="POST">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control @error('immatriculation') is-invalid @enderror" 
                                   name="immatriculation" id="immatriculation" value="{{ old('immatriculation') }}" required>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="client_id" id="client_id" class="form-select @error('client_id') is-invalid @enderror">
                                ...
                                @foreach($clients as $client)
                                    <option value="{{ $client->id_client }}" {{ old('client_id') == $client->id_client ? 'selected' : '' }}>
                                        {{ $client->nom }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="sim_id" id="sim_id" class="form-select">
                                ...
                                @foreach($sims as $sim)
                                    <option value="{{ $sim->id_sim }}" {{ old('sim_id') == $sim->id_sim ? 'selected' : '' }}>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3" id="raison_suspension_group" style="display: none;">
                            <label for="raison_suspension" class="form-label">Raison de suspension *</label>
                            <textarea name="raison_suspension" id="raison_suspension" class="form-control" rows="3">{{ old('raison_suspension') }}</textarea>
                        </div>
                    </div>
...
<script>
    ...
    if (document.getElementById('statut').value === 'suspendu') {
        document.getElementById('raison_suspension_group').style.display = 'block';
    }
</script>
@endsection
